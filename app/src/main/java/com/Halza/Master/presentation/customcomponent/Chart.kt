...
fun Chart(
    ...
    Box(
        ...
        Canvas(modifier = Modifier
            ...
            .padding(
                top = 50.dp, bottom = 20.dp, start = 30.dp, end = 30.dp
            )
            .pointerInput(Unit) {
            ...
            for (item in data) {
                val itemValue = if (item.value >= target) target else item.value
                val topLeft = Offset(
                    x = spaceStep, y = size.height - itemValue * barScale - labelOffset
                )
                //--------------------(draw bars)--------------------//
                drawRoundRect(
                    color = if (item.value >= target) barColor
                ...
                drawContext.canvas.nativeCanvas.drawText(
                    if (!item.key.toString()
                            .contains("md", ignoreCase = false)
                    ) item.key.toString() else "",
                    spaceStep + barWidth / 2, size.height, paint
                )
